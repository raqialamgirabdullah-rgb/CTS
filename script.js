...
const showLogoToggle = $('showLogoToggle');
...
const colorKeyPreview = $('colorKeyPreview');
const colorKeyTolerance = $('colorKeyTolerance');
const colorKeyToleranceLabel = $('colorKeyToleranceLabel');
const colorKeyStatus = $('colorKeyStatus');
...
let previousTool = 'none';  // tool to restore after a one-shot color pick

// color-key (chroma) background removal state
let basisData = null;       // clean snapshot of the photo, taken fresh each initEditCanvas()
let keyColor = null;        // {r,g,b} sampled background color to remove
...
function initEditCanvas() {
  ...
  basisData = editCtx.getImageData(0, 0, editCanvas.width, editCanvas.height);
  keyColor = null;
  colorKeyPreview.style.background = 'var(--paper)';
  colorKeyStatus.textContent = 'Tap "Pick Color", then tap the background area of the photo above.';
}
...
// default logo is hosted on GitHub Pages (see the URL below). This keeps the
// image out of this JS file entirely — if you move repos, just update the URL.
(function loadDefaultLogo() {
  const url = 'https://raqialamgirabdullah-rgb.github.io/CTS/default-logo.webp';
  const img = new Image();
  img.crossOrigin = 'anonymous'; // GitHub Pages sends CORS headers; without this the
                                  // canvas gets "tainted" and downloads silently fail
  img.onload = () => {
    logoImg = img;
  ...
  img.onerror = () => {
    // retry once without crossOrigin, so the logo still shows even if this
    // particular fails — downloads just won't work until a logo is re-uploaded
    const fallback = new Image();
    fallback.onload = () => {
      logoImg = fallback;
      logoThumb.src = fallback.src;
      logoThumb.classList.add('show');
      render();
    };
    fallback.src = url;
...
showLogoToggle.addEventListener('change', render);
...
/* ---------- flat-color background removal (chroma key) ---------- */
// Always recomputed from the clean `basisData` snapshot, so re-picking a
// color or dragging the tolerance slider never compounds on itself.
function applyColorKey() {
  if (!keyColor || !basisData) return;
  const w = editCanvas.width;
  const h = editCanvas.height;
  const src = basisData.data;
  const out = new Uint8ClampedArray(src); // fresh copy each time
  const tol = (colorKeyTolerance.value / 100) * 441.7; // max RGB distance ≈ sqrt(255²×3)
  for (let i = 0; i < out.length; i += 4) {
    const dr = out[i] - keyColor.r;
    const dg = out[i + 1] - keyColor.g;
    const db = out[i + 2] - keyColor.b;
    if (Math.sqrt(dr * dr + dg * dg + db * db) <= tol) out[i + 3] = 0;
  }
  editCtx.putImageData(new ImageData(out, w, h), 0, 0);
  render();
}
...
pickColorBtn.addEventListener('click', () => {
  ...
  colorKeyStatus.textContent = 'Tap the background area of the photo above.';
});

wireRange(colorKeyTolerance, colorKeyToleranceLabel, '');
colorKeyTolerance.addEventListener('input', applyColorKey);
...
canvas.addEventListener('pointerdown', (e) => {
  ...
  if (tool === 'sample') {
    const px = Math.min(editCanvas.width - 1, Math.max(0, Math.round(pos.x)));
    const py = Math.min(editCanvas.height - 1, Math.max(0, Math.round(pos.y)));
    const d = editCtx.getImageData(px, py, 1, 1).data;
    keyColor = { r: d[0], g: d[1], b: d[2] };
    colorKeyPreview.style.background = `rgb(${d[0]},${d[1]},${d[2]})`;
    colorKeyStatus.textContent = `Picked rgb(${d[0]}, ${d[1]}, ${d[2]}) — adjust tolerance to fine-tune.`;
    applyColorKey();
    tool = previousTool;
    canvas.classList.toggle('tool-active', tool !== 'none');
    return;
  }
...
function render() {
  ...
  if (logoImg && showLogoToggle.checked) {
    const margin = W * 0.03;
